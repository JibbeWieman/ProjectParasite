...
public class CameraShake : MonoBehaviour
{
    ...
    protected CinemachineFreeLook m_CinemachineFlCam;
    protected bool m_IsShaking = false;
    protected Vector3 m_OriginalLocalPosition;

    private void Awake()
    {
        m_CinemachineFlCam = GetComponent<CinemachineFreeLook>();
    }
    ...
    private void LateUpdate()
    {
        if (m_IsShaking)
        {
            m_CinemachineFlCam.LookAt.localPosition = m_OriginalLocalPosition + Random.insideUnitSphere * m_ShakeAmount;

            m_RemainingShakeTime -= Time.deltaTime;
            if (m_RemainingShakeTime <= 0)
            {
                m_IsShaking = false;
                m_CinemachineFlCam.LookAt.localPosition = m_OriginalLocalPosition;
            }
        }
    }
    ...
    private void StartShake(float amount, float time)
    {
        if (!m_IsShaking)
        {
            m_OriginalLocalPosition = m_CinemachineFlCam.LookAt.localPosition;
        }
